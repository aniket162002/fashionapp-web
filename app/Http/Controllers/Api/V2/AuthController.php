...
class AuthController extends Controller
{
    public function signup(Request $request)
    {
        ...
        // Ensure default role for app users
        ...
        if ($request->register_by == 'email') {
            ...
            $emailVerificationEnabled = BusinessSetting::where('type', 'email_verification')->first();
            if (!$emailVerificationEnabled || $emailVerificationEnabled->value != 1) {
        ...
        if ($request->register_by == 'phone') {
            ...
            // For phone registration, phone is already verified via Firebase OTP
            // So we mark the user as verified immediately
            $user->email_verified_at = date('Y-m-d H:i:s');
        }
        
        $user->save();

        // Ensure customer profile exists for shopping flows IF a separate `customers` table exists
        if (Schema::hasTable('customers')) {
            Customer::firstOrCreate(['user_id' => $user->id]);
        }

        // Send verification notifications only if needed
    ...
    public function login(Request $request)
    {
        ...
        if ($delivery_boy_condition) {
            ...
            $user = User::whereIn('user_type', ['customer'])
                ->where(function ($query) use ($req_email, $request) {
                    if ($request->login_by == 'phone') {
                        $query->where('phone', $req_email);
    ...
    {
        if (!$token) {
            // Create token with expiration (optional - set to null for no expiration)
            $tokenResult = $user->createToken('API Token');
            $token = $tokenResult->plainTextToken;
            
            // Optionally set expiration (uncomment if needed)
