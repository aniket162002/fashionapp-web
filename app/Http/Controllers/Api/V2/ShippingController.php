      ...
      public function getDeliveryInfo(Request $request)
{
    ...
    $owner_ids = $cartItems->select('owner_id', 'product_id')->groupBy('owner_id')->get();
    // dd($owner_ids);
    $shops = [];
    if (!empty($owner_ids)) {
        foreach ($owner_ids as $owner_id) {
            // dd($owner_id->owner_id);
            $shop = array();

            $shop_items_raw_data = $cartItems->where('owner_id', $owner_id->owner_id)->get()->toArray();
            ...
            if (!empty($shop_items_raw_data)) {
                foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                    if (isset($shop_items_raw_data_item["product_id"])) {

                        $product = Product::where('id', $shop_items_raw_data_item["product_id"])->first();
                        // dd($product);
                        if ($product) {
                            $shop_items_data_item = [];
                            $shop_items_data_item["id"] = isset($shop_items_raw_data_item["id"]) ? intval($shop_items_raw_data_item["id"]) : null;
                            $shop_items_data_item["owner_id"] = isset($shop_items_raw_data_item["owner_id"]) ? intval($shop_items_raw_data_item["owner_id"]) : null;
                            $shop_items_data_item["user_id"] = isset($shop_items_raw_data_item["user_id"]) ? intval($shop_items_raw_data_item["user_id"]) : null;
                            $shop_items_data_item["temp_user_id"] = isset($shop_items_raw_data_item["temp_user_id"]) ? intval($shop_items_raw_data_item["temp_user_id"]) : null;
                            $shop_items_data_item["product_id"] = isset($shop_items_raw_data_item["product_id"]) ? intval($shop_items_raw_data_item["product_id"]) : null;
            ...
            $shop_data = Shop::where('user_id', $owner_id->owner_id)->first();
            // dd($shop_data);
            if ($shop_data) {
                $shop['name'] = $shop_data->name;
                $shop['owner_id'] = (int) $owner_id->owner_id; // Ensure this is an integer
            ...
            $shop['carriers'] = seller_base_carrier_list($owner_id->owner_id, $userId, $tempUserId, $shipping_info);
            $shop['pickup_points'] = [];
