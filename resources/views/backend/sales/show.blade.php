                ...
                @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
                    ...
                    <div class="col-md-3 ml-auto">
                        <label for="update_tracking_code">
                            {{ translate('Tracking URL (optional)') }}
                        </label>
                        <input type="url" class="form-control" id="update_tracking_code"
                            placeholder="https://carrier.example.com/track/ABC123"
                ...
                <div class="col-md-4 ml-auto">
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <td class="text-main text-bold">{{ translate('Tracking URL') }}</td>
                                <td class="text-right">
                                    @if(filter_var($order->tracking_code, FILTER_VALIDATE_URL))
                                        <a href="{{ $order->tracking_code }}" target="_blank" class="btn btn-sm btn-soft-info">
                                            <i class="las la-external-link-alt"></i> {{ translate('Track Order') }}
                                        </a>
                                    @else
                                        <span class="text-muted">{{ $order->tracking_code }}</span>
                                    @endif
                                </td>
                            </tr>
                            @endif
...
@section('script')
    <script type="text/javascript">
        ...
        $('#update_tracking_code').on('change', function() {
            ...
            if (tracking_code && !isValidUrl(tracking_code)) {
                AIZ.plugins.notify('danger', '{{ translate('Please enter a valid URL (e.g., https://example.com/track/123)') }}');
                return;
            }
            ...
            $.post('{{ route('orders.update_tracking_code') }}', {
                ...
                AIZ.plugins.notify('success', '{{ translate('Order tracking URL has been updated') }}');
            });
        });
